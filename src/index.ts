...
const API_BASE = "https://gamesdb.launchbox-app.com";
const API_KEY_ENV = "LAUNCHBOX_API_KEY";
...
export class LaunchBoxProvider implements MetadataProvider {
  ...
  async search(query: string): Promise<MetadataSearchResult[]> {
    const url = new URL(`${API_BASE}/Games/Search`);
  ...
  async getDetails(id: string): Promise<MetadataDetails | null> {
    const url = new URL(`${API_BASE}/Games/Details/${id}`.replace("${id}", id));
    const payload = await this.request(url);
  ...
  private async request(url: URL): Promise<unknown> {
    ...
    if (!response.ok) {
      throw new Error(`LaunchBox Games Database request failed: ${response.status}`);
    }
...
export default class LaunchBoxPlugin implements ServerPlugin {
  ...
  async init(ctx: PluginContext): Promise<void> {
    ...
    ctx.registerMetadataProvider(
      new LaunchBoxProvider(apiKey, ctx.fetch.bind(ctx)),
    );
    ctx.logger.info(
      `LaunchBox Games Database metadata provider registered${apiKey ? "" : " (no API key configured)"}`,
    );
  }
}
